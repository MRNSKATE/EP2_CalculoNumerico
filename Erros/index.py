...
function = str(input("Digite uma função: "))
x_inf = float(input('Digite o valor inferior do intervalo externo: '))
x_sup = float(input('Digite o valor superior do intervalo externo: '))
...
while validation == True:
    ...
    if loop_validation == 's':
        ...
        for i in range(len(x_j)):
            ...
            new_function = str(((new_ysup_value - new_yinf_value)/2)*w_j[i])+'*('+transform_y(function, str(y_x_i))+')'
        ...
        print(f'\n O valor da sua integral é: {"%.3f" % total_value}\n')
        count = True
